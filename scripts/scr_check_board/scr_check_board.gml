/// @description  Checks each piece on the board for matches.
function scr_check_board(argument0) {
	/* argument0 -- if true, matching sets of three or more are designated
	...
	*/
	...
	{
	array_pieces[i + h_count, j].matched =
	true;
	}
	h_count--;
	}
	...
	if (!argument0)
	{
	array_pieces[i, j + v_count].image_index =
	irandom(piece_range);
	}
	else
	{
